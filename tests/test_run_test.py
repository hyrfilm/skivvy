...
def test_fortune_01_successful(httpserver):
    httpserver.expect_request("/api/fortune/1").respond_with_json({"wisdom": "If it seems that fates are aginst you today, they probably are."})
    status, error_context = run_test("./tests/fixtures/testcases/check_status.json", default_cfg)
...
def test_fortune_01_failing(httpserver):
    ...
    httpserver.expect_request("/api/fortune/1").respond_with_json({"wisdom": "If it seems that fates are aginst you today, they probably are."})
    status, error_context = run_test("./tests/fixtures/testcases/check_exact_match.json", default_cfg)
...
def test_fortune_02_match_subsets(httpserver):
    httpserver.expect_request("/api/fortune").respond_with_json({"wisdom": "You only live once."})
    status, error_context = run_test("./tests/fixtures/testcases/match_subset.json", default_cfg)
    assert status is STATUS_OK
    #assert error_context is None
