...
from skivvy.matchers import brace_expand, file_writer
...
def test_simple_expansion(tmp_files):
    assert brace_expand("<user_id.txt>", auto_coerce=False) == "123"
    assert brace_expand("<user_id.txt>", auto_coerce=True) == 123

def test_compound_expansion(tmp_files):
    assert "www.example.com/dashboard/user/123" == brace_expand("www.example.com/<dashboard.txt>/<user_id.txt>", auto_coerce=True)
...
def test_brace_expansion_with_variables():
    ...
    request = test_util.json_transform_str(template, brace_expand)
    expected = {
    ...
    brace_expand_coerce = partial(brace_expand, auto_coerce=True)
    ...
    assert dict_util.map_nested_dicts_py(new_dict, brace_expand) == new_dict
    ...
    assert test_util.json_transform_str({ "a": "<email>", "b": "<session>" }, brace_expand) == expected
